...
for model in model_list:
    # loop through each order and construct the parameters for each run
    for order in orders:
        print("---------------------------------------------------------------------------------------------------------------------")
        print("---------------------------------------------------------------------------------------------------------------------")
        print(f"- working on model {model} for order hash {order['order_hash']} -")
        print("---------------------------------------------------------------------------------------------------------------------")
        print("---------------------------------------------------------------------------------------------------------------------")

        # #####################################################################################
        # set the parameters for each run
        # #####################################################################################
        model_view_url = order["model_view_url"]
        model_input_file_path = order["model_data_url"] # data file url
        model_output_file_path = order[model_list[model]]
        network = order['network']
        target_order_hash = order['order_hash']
        start_date_str = order['strategy_start_date']
        end_date_str = order['strategy_lasttrade_date']
        subgraph_url = subgraph_urls[network]
        rows_to_skip = 22

        # force the date string formats to be: %Y-%m-%d
        start_date_str = parse_flexible_date(start_date_str, "%Y-%m-%d")
        end_date_str = parse_flexible_date(end_date_str, "%Y-%m-%d")

        print("\n--------------------------------\nInput parameters\n--------------------------------\n")
        print(f"model view url: {model_view_url}")
        ...
        validation_outputs.append(
            {
                ...
                "strategy_start_date": strategy_start_datetime,
                "strategy_last_trade_date": strategy_trades_date_end,

                # Unpack the strategy_basic_stats dictionary
...
# print info about the df
print("\n--------------------------------\nValidation outputs dataframe info\n--------------------------------\n")
print(df_validation_outputs.info())
